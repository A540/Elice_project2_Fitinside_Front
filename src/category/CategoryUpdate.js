...
const CategoryUpdate = () => {
    ...
    useEffect(() => {
        // 부모 카테고리 목록 가져오기
        axios.get('http://3.95.202.193:8080/api/categories', {
            headers: {
        ...
        // 선택된 카테고리 정보 가져오기
        axios.get(`http://3.95.202.193:8080/api/categories/${id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        })
    ...
    const handleSubmit = (e) => {
        ...
        axios.put(`http://3.95.202.193:8080/api/admin/categories/${id}`, formData, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
