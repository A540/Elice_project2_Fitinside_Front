...
const ProductAdmin = () => {
    ...
    const fetchProducts = async (pageNumber) => {
        ...
        try {
            ...
            const response = await axios.get(`http://3.95.202.193${endpoint}`, {
                headers: {
            ...
            if (error.response && error.response.status === 401) {
                ...
                try {
                    ...
                    // 토큰 갱신 후 다시 요청
                    const newResponse = await axios.get(`http://3.95.202.193${endpoint}`, {
                        headers: {
    ...
    const handleDeleteClick = async (productId) => {
        ...
        try {
            ...
            const response = await axios.delete(`http://3.95.202.193/api/admin/products/${productId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,  // Authorization 헤더 추가
                },
            });
            ...
            if (error.response && error.response.status === 401) {
                ...
                try {
                    ...
                    // 토큰 갱신 후 다시 삭제 요청
                    const newResponse = await axios.delete(`http://3.95.202.193/api/admin/products/${productId}`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`,  // 새로 발급된 토큰 사용
                        },
                    });
