...
const OrderCreate = () => {
    ...
    useEffect(() => {
        ...
        const fetchProductDetails = async () => {
            ...
            for (const item of storedOrderData) {
                try {
                    const response = await axios.get(`http://3.95.202.193:8080/api/products/${item.productId}`, {
                        headers: {
        ...
        const fetchDefaultAddress = async () => {
            try {
                const response = await axios.get('http://3.95.202.193:8080/api/addresses/default', {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    const addAddress = async (deliveryData) => {
        try {
            ...
            const addressResponse = await axios.post('http://3.95.202.193:8080/api/addresses', deliveryData, {
                headers: {
    ...
    const updateAddress = async (deliveryData) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.patch(`http://3.95.202.193:8080/api/addresses/${selectedAddressId}`, deliveryData, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const createOrder = async (deliveryData) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('http://3.95.202.193:8080/api/order', {
