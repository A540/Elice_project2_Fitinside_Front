...
const OrderDetail = () => {
    ...
    useEffect(() => {
        ...
        const fetchOrderDetail = async () => {
            try {
                ...
                const response = await axios.get(`http://3.95.202.193:8080/api/orders/${orderId}`, {
                    headers: {
    ...
    const handleSaveClick = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.patch(`http://3.95.202.193:8080/api/orders/${orderId}`, updatedData, {
                headers: {
    ...
    const handleCancelClick = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://3.95.202.193:8080/api/orders/${orderId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
