...
const MemberList = () => {
    ...
    useEffect(() => {
        ...
        const fetchMembers = async (page) => {
            try {
                ...
                const response = await axios.get(`http://fitinside.duckdns.org:8080/api/admin/member?page=${page}`, {
                    headers: {
    ...
    const handleDeleteMember = async (memberId) => {
        try {
            ...
            const token = localStorage.getItem('token'); // 토큰 가져오기
            await axios.delete(`http://fitinside.duckdns.org:8080/api/admin/member/${memberId}`, {
                headers: {
                    Authorization: `Bearer ${token}`, // 토큰을 Authorization 헤더에 포함
                },
            });
