...
const CouponSearchModal = ({ isOpen, onRequestClose }) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await axios.get(`https://fitinside.duckdns.org/api/coupons/code/${couponCode}`, {
                headers: {
    ...
    const handleCouponSubmit = async () => {
        ...
        try {
            const response = await axios.post('https://fitinside.duckdns.org/api/coupons', couponCode, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
