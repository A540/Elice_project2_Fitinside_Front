...
const CouponSearchModal = ({ isOpen, onRequestClose }) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await axios.get(`http://3.95.202.193:8080/api/coupons/code/${couponCode}`, {
                headers: {
    ...
    const handleCouponSubmit = async () => {
        ...
        try {
            const response = await axios.post('http://3.95.202.193:8080/api/coupons', couponCode, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
