...
const CouponEmailModal = ({ isOpen, onRequestClose, coupon }) => {
    ...
    const fetchMembers = async (id) => {
        ...
        setError(null);
        try {
            const response = await axios.get(`http://3.95.202.193:8080/api/admin/coupons/${id}/members`, {
                headers: {
    ...
    const sendCouponEmails = async () => {
        ...
        try {
            await Promise.all(selectedEmails.map(async (email, index) => {
                ...
                // 실제 이메일 전송 요청
                const response = await axios.post('http://3.95.202.193:8080/api/admin/coupons/email', {
                    couponId: coupon.id,
                    address: email,
