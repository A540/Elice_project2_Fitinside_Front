...
const CouponList = () => {
    ...
    const fetchCoupons = async (page, includeInActiveCoupons) => {
        try {
            const response = await axios.get(`http://fitinside.duckdns.org:8080/api/coupons`, {
                params: {
                    page: page,
    ...
    const handleOrderHistoryClick = async (couponId) => {
        try {
            const response = await axios.get(`http://fitinside.duckdns.org:8080/api/coupons/${couponId}/order`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
