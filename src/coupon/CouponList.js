...
const CouponList = () => {
    ...
    const fetchCoupons = async (page, includeInActiveCoupons) => {
        try {
            const response = await axios.get(`https://fitinside.duckdns.org/api/coupons`, {
                params: {
                    page: page,
    ...
    const handleOrderHistoryClick = async (couponId) => {
        try {
            const response = await axios.get(`https://fitinside.duckdns.org/api/coupons/${couponId}/order`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
