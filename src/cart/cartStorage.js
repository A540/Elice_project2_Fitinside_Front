...
export const fetchProduct = async (id) => {
    try {
        const response = await axios.get(`http://3.95.202.193/api/products/${id}`, {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get(`http://3.95.202.193/api/products/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
...
export const fetchAndMergeCartData = async (token) => {
    try {
        const response = await axios.get('http://3.95.202.193/api/carts', {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get('http://3.95.202.193/api/carts', {
                headers: {
...
const updateDBWithDifferences = async (localCart) => {
    ...
    for (const item of localCart) {
        ...
        if (!dbItem) {
            ...
            try {
                await axios.post('http://3.95.202.193/api/carts', requestData, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const updateCartItem = async (item) => {
    ...
    try {
        await axios.put('http://3.95.202.193/api/carts', requestData, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
...
const deleteCartItem = async (item) => {
    ...
    if(localStorage.getItem('token') === null) return;
    try {
        await axios.delete(`http://3.95.202.193/api/carts/${item.id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
